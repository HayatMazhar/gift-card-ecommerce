...
namespace Bamboo.DataAccess.Common
{
    public class BaseRepositoryAsync<T> where T : class
    {
        ...
        {
            return await _connection.ExecuteScalarAsync<T>(sqlQuery, parameters, _transaction);
        }

        public async Task<int> ExecuteAsync(string sqlQuery, T entity)
        {
            return await _connection.ExecuteAsync(sqlQuery, entity, _transaction);
        }
        public async Task<int> ExecuteAsync(string sqlQuery, DynamicParameters parameters)
        {
            return await _connection.ExecuteAsync(sqlQuery, parameters, _transaction);
        }
        ...
        public async Task<List<dynamic>> GetAllByStoredProcedureDynamicAsync(string storedProcedureName, DynamicParameters parameters)
        {
            return (await _connection.QueryAsync<dynamic>(storedProcedureName, parameters, _transaction, commandType: CommandType.StoredProcedure)).ToList();
        }
    }
}
